var Tree=function(){};Tree.treeReload=function(id,items){var shape,$elem=$('#adam_tree');$elem.empty();$elem.pmtree({id:id,collapsed:true,items:items,select:function(param){shape=jCore.getActiveCanvas().customShapes.find('id',param.uid);if(shape){shape.canvas.emptyCurrentSelection();shape.canvas.addToSelection(shape);if(shape.canvas.currentLabel){shape.canvas.currentLabel.loseFocus();}
shape.canvas.project.updatePropertiesGrid(shape);}},unselect:function(id){}});};var setSelectedNode=function(shape){var id="#"+$('a[uid ="'+shape.getID()+'"]').attr("desc");$(".treechild").attr("status","unmarked");$(".treechild").css("background","#fff");$(id).css("background","#CEE3F6");$(id).attr("status","marked");};
/* End of File include/javascript/pmse/tree.js */
...
/* End of File include/javascript/pmse/drag_behavior.js */
...
/* End of File include/javascript/pmse/drop_behavior.js */

var AdamShape=function(options){jCore.CustomShape.call(this,options);this.label=this.labels.get(0);this.markersArray=new jCore.ArrayList();this.hasWarning=false;this.hasError=false;};AdamShape.prototype=new jCore.CustomShape();AdamShape.prototype.type="AdamShape";AdamShape.prototype.adamConnectionDropBehavior=null;AdamShape.prototype.getType=function(){return this.type;};AdamShape.prototype.setName=function(value){var item;if(this.label){this.label.setMessage(value);}
return this;};AdamShape.prototype.saveProject=function(root,App,w){root.canvas.showModal();App.alert.show('upload',{level:'process',title:'LBL_LOADING_NO_DOTS',autoClose:false});root.canvas.project.save({success:function(){root.canvas.hideModal();w.show();w.html.style.display='none';}});};AdamShape.prototype.getName=function(){var text="";if(this.label){text=this.label.getMessage();}
return text;};AdamShape.prototype.getContextMenu=function(){return{};};AdamShape.prototype.getContextMenuCallbacks=function(){return{};};AdamShape.prototype.setDimension=function(x,y){var factor;jCore.CustomShape.prototype.setDimension.call(this,x,y);if(this.getType()==='AdamEvent'||this.getType()==='AdamGateway'){factor=3;}else{if(this.getType()==='AdamActivity'&&this.act_task_type==='SCRIPTTASK'){factor=3;}else{factor=1;}}
if(this.label){this.label.setDimension((this.zoomWidth*0.9*factor)/ this.canvas.zoomFactor,this.label.height);this.label.setLabelPosition(this.label.location,this.label.diffX,this.label.diffY);}
return this;};AdamShape.prototype.dragBehaviorFactory=function(type){if(type==="regular"){if(!this.regularDragBehavior){this.regularDragBehavior=new jCore.RegularDragBehavior();}
return this.regularDragBehavior;}
if(type==="connection"){if(!this.connectionDragBehavior){this.connectionDragBehavior=new AdamConnectionDragBehavior();}
return this.connectionDragBehavior;}
if(type==="customshapedrag"){if(!this.customShapeDragBehavior){this.customShapeDragBehavior=new AdamShapeDragBehavior();}
return this.customShapeDragBehavior;}
if(!this.noDragBehavior){this.noDragBehavior=new jCore.NoDragBehavior();}
return this.noDragBehavior;};AdamShape.prototype.dropBehaviorFactory=function(type,selectors){if(type==="nodrop"){if(!this.noDropBehavior){this.noDropBehavior=new jCore.NoDropBehavior(selectors);}
return this.noDropBehavior;}
if(type==="container"){if(!this.containerDropBehavior){this.containerDropBehavior=new jCore.ContainerDropBehavior(selectors);}
return this.containerDropBehavior;}
if(type==="connection"){if(!this.connectionDropBehavior){this.connectionDropBehavior=new AdamConnectionDropBehavior(selectors);}
return this.connectionDropBehavior;}
if(type==="connectioncontainer"){if(!this.connectionContainerDropBehavior){this.connectionContainerDropBehavior=new AdamConnectionContainerDropBehavior(selectors);}
return this.connectionContainerDropBehavior;}};AdamShape.prototype.getSourceElements=function(){var elements=[];var i;var port;var connection;for(i=0;i<this.getPorts().getSize();i+=1){port=this.getPorts().get(i);connection=port.connection;if(connection.srcPort.parent.getID()!==this.getID()){elements.push(connection.srcPort.parent);}}
return elements;};AdamShape.prototype.getDestElements=function(){var elements=[],i,port,connection;for(i=0;i<this.getPorts().getSize();i+=1){port=this.getPorts().get(i);connection=port.connection;if(connection.srcPort.parent.getID()===this.getID()){elements.push(connection.destPort.parent);}}
return elements;};AdamShape.prototype.getDestElementName=function(floID){var i;var flow;var ports=this.getPorts().asArray();for(i=0;i<ports.length;i++){flow=ports[i].connection;if(flow.flo_uid===floID){return flow.destPort.parent.getName();}}};AdamShape.prototype.setDefaultFlow=function(floID){var i,port,connection;for(i=0;i<this.getPorts().getSize();i+=1){port=this.getPorts().get(i);connection=port.connection;this.updateDefaultFlow(0);if(connection.srcPort.parent.getID()===this.getID()){if(connection.getID()===floID){this.updateDefaultFlow(floID);connection.setFlowCondition("");connection.changeFlowType('default');connection.setFlowType("DEFAULT");}else if(connection.getFlowType()==='DEFAULT'){connection.changeFlowType('sequence');connection.setFlowType("SEQUENCE");}}}
return this;};AdamShape.prototype.updateFlowConditions=function(){var i,connection,updatedElement;for(i=0;i<this.getPorts().getSize();i+=1){connection=this.getPorts().get(i).connection;if(connection.flo_element_origin===this.getID()){if(connection.flo_condition&&connection.flo_condition!==''){updatedElement=[{id:connection.getID(),type:connection.type,relatedObject:connection,fields:[{field:"condition",newVal:'',oldVal:connection.getFlowCondition()}]}];connection.setFlowCondition('');connection.getCanvas().triggerDefaultFlowChangeEvent(updatedElement);}}}};AdamShape.prototype.getFamilyNumber=function(shape){var map={'AdamActivity':5,'AdamEvent':6,'AdamGateway':7,'AdamData':8,'AdamArtifact':9};return map[shape.getType()];};AdamShape.prototype.getShapeType=function(type,shape){var shapeType,shapeMessage,itemType="";switch(type){case"AdamActivity":shapeType=shape.act_task_type;itemType=type+shapeType;break;case"AdamEvent":shapeType=shape.getEventType();shapeMessage=shape.getEventMessage()||shape.getEventMarker();itemType=type+shapeType+shapeMessage;break;case"AdamGateway":shapeType=shape.getGatewayType();shapeMessage=shape.getGatewayType();itemType=type+shapeType;break;};return itemType;};AdamShape.prototype.countFlow=function(element,direction){var i,eleMap={'sequenceFlow':'regular','associationFlow':'dotted'},port,connection,count=0;for(i=0;i<this.getPorts().getSize();i+=1){port=this.getPorts().get(i);connection=port.getConnection();switch(direction){case'incoming':if(eleMap[element]===connection.segmentStyle){if(port.getID()===connection.getDestPort().getID()){count+=1;}}
break;case'outgoing':if(eleMap[element]===connection.segmentStyle){if(port.getID()===connection.getSrcPort().getID()){count+=1;}}break;case'none':if(port.getID()===connection.getSrcPort().getID()){count+=1;}
break;}}
return count;};AdamShape.prototype.showWarningMarker=function(){this.addErrorLayer('error',5,true);};AdamShape.prototype.showErrorMarker=function(){this.addErrorLayer('error',2);};AdamShape.prototype.clearIssueMarkers=function(){var i;var lMarker;for(i=0;i<this.markersArray.getSize();i+=1){lMarker=this.markersArray.get(i);if(lMarker.position===2||lMarker.position===5){$('#'+lMarker.id).remove();this.markersArray.remove(lMarker);i--;}}};AdamShape.prototype.addErrorLayer=function(cssMarker,position,warning){var layer,cl,cs,zoom,options;layer=this.layers.find('id',this.id+'Layer-error-layer');if(typeof position==='undefined'||position===null){cl=cssMarker;cs='bpmn_zoom';}else{cl='div-empty';cs='';}
if(typeof layer==='undefined'){options={layerName:"error-layer",priority:3,visible:true,style:{cssClasses:[]}};layer=this.createLayer(options);}else{if(typeof position==='undefined'||position===null){layer.setElementClass(cl);}}
if(typeof position!=='undefined'&&position!==null){this.addErrors(layer,position,warning);}};AdamShape.prototype.addErrors=function(newLayer,pos,warning){var nMarker,x,lMarker,ifExist=false,errorArrayClass=[],cls,i;for(i=0;i<this.markersArray.getSize();i+=1){lMarker=this.markersArray.get(i);if(lMarker.position===pos){ifExist=true;break;}}
for(i=0;i<newLayer.ZOOMSCALES;i+=1){if(warning){cls='element-zoom-'+((i*25)+50)+'-marker adam-warning-color fa fa-exclamation-triangle';}else{cls='element-zoom-'+((i*25)+50)+'-marker adam-error-color fa fa-exclamation-circle';}
errorArrayClass.push(cls);}
if(!ifExist){nMarker=new AdamMarker({parent:newLayer,position:pos,height:17,width:17,markerZoomClasses:errorArrayClass});this.markersArray.insert(nMarker);nMarker.paint();nMarker.setElementClass(errorArrayClass);}else{lMarker.setElementClass(errorArrayClass);}};AdamShape.prototype.validate=function(validationTools){var self=this;var url=App.api.buildURL(this.getBaseURL()+self.id,null,null);var options={'bulk':'get_element_settings'};var callback=self.getValidationFunction();self.hasWarning=false;self.hasError=false;if(url&&callback){validationTools.progressTracker.incrementTotalElements();App.api.call('read',url,null,{success:function(data){callback(data,self,validationTools);},error:function(data){validationTools.createWarning(element,'LBL_PMSE_ERROR_UNABLE_TO_VALIDATE',self.getName());},complete:function(data){validationTools.progressTracker.incrementSettingsGathered();}},options);}};
/* End of File include/javascript/pmse/shapes.js */
...
return this.html;};
/* End of File include/javascript/pmse/flow.js */

var CommandAdam=function(receiver,propertyNames,newValues){var oldValues=[],i;jCore.Command.call(this,receiver);for(i=0;i<propertyNames.length;i+=1){oldValues.push(receiver[propertyNames[i]]);}
this.oldValues=oldValues;this.newValues=newValues;this.propertyNames=propertyNames;};CommandAdam.prototype=new jCore.Command();CommandAdam.prototype.type='CommandAdam';CommandAdam.prototype.execute=function(){var e;for(e=0;e<this.newValues.length;e+=1){this.receiver[this.propertyNames[e]]=this.newValues[e];}
this.canvas.triggerCommandAdam(this.receiver,this.propertyNames,this.oldValues,this.newValues);};CommandAdam.prototype.undo=function(){var e;for(e=0;e<this.newValues.length;e+=1){this.receiver[this.propertyNames[e]]=this.oldValues[e];}
this.canvas.triggerCommandAdam(this.receiver,this.propertyNames,this.newValues,this.oldValues);};CommandAdam.prototype.redo=function(){this.execute();};var AdamShapeLayerCommand=function(receiver,options){jCore.Command.call(this,receiver);this.layers=[];this.updateType=null;this.beforeStyle=[];this.beforeValues=[];this.afterStyle=[];this.afterValues=[];this.beforeName=null;this.afterName=null;this.propertyNames=[];AdamShapeLayerCommand.prototype.initObject.call(this,receiver,options);};AdamShapeLayerCommand.prototype=new jCore.Command();AdamShapeLayerCommand.prototype.type="AdamShapeLayerCommand";AdamShapeLayerCommand.prototype.initObject=function(receiver,options){var i,newZoom,css,marker,type;this.updateType=options.type;this.layers=options.layers;switch(this.updateType){case'changetypegateway':this.beforeStyle.push(this.layers[0].zoomSprites);newZoom=[];for(i=0;i<this.beforeStyle[0].length;i+=1){newZoom.push('adam-shape-'+((i*25)+50)+'-gateway-'+options.changes.toLowerCase());}
this.afterStyle.push(newZoom);this.propertyNames.push('gat_type');this.beforeValues.push(receiver['gat_type']);this.afterValues.push(options.changes);if(parseInt(receiver['gat_default_flow'])!==0&&(options.changes==='PARALLEL'||options.changes==='EVENTBASED')){this.propertyNames.push('gat_default_flow');this.beforeValues.push(receiver['gat_default_flow']);this.afterValues.push(0);}
if(options.changes==='EVENTBASED'){this.propertyNames.push('gat_direction');this.beforeValues.push(receiver['gat_direction']);this.afterValues.push('UNSPECIFIED');}
break;case'changeeventmarker':this.beforeStyle.push(this.layers[0].zoomSprites);newZoom=[];marker=(options.changes.evn_message&&(options.changes.evn_message!==""))?options.changes.evn_message:options.changes.evn_marker;type=(this.receiver.evn_type==="BOUNDARY")?'INTERMEDIATE':this.receiver.evn_type;for(i=0;i<this.beforeStyle[0].length;i+=1){css='adam-marker-'+((i*25)+50)+'-'+type.toLowerCase();css+='-'+options.changes.evn_behavior.toLowerCase()+'-';css+=marker.toLowerCase();newZoom.push(css);}
this.afterStyle.push(newZoom);if(typeof options.changes.evn_behavior!=='undefined'){this.propertyNames.push('evn_behavior');this.beforeValues.push(this.receiver['evn_behavior']);this.afterValues.push(options.changes.evn_behavior);}
if(typeof options.changes.evn_marker!=='undefined'){this.propertyNames.push('evn_marker');this.beforeValues.push(this.receiver['evn_marker']);this.afterValues.push(options.changes.evn_marker);}
this.beforeName=this.afterName=receiver.getName();if(options.changes.evn_name!==undefined){this.afterName=options.changes.evn_name;}
this.propertyNames.push('evn_message');this.beforeValues.push(this.receiver['evn_message']);this.afterValues.push(options.changes.evn_message||'');break;case'changeeventtype':this.beforeStyle.push(this.layers[0].zoomSprites);newZoom=[];for(i=0;i<this.beforeStyle[0].length;i+=1){newZoom.push('adam-shape-'+((i*25)+50)+'-event-'+options.changes.evn_type.toLowerCase());}
this.afterStyle.push(newZoom);newZoom=[];this.beforeStyle.push(this.layers[1].zoomSprites);for(i=0;i<this.beforeStyle[1].length;i+=1){css='adam-marker-'+((i*25)+50)+'-';css+=options.changes.evn_type.toLowerCase()+'-';css+=options.changes.evn_behavior.toLowerCase()+'-';css+=options.changes.evn_marker.toLowerCase();newZoom.push(css);}
this.afterStyle.push(newZoom);if(typeof options.changes.evn_type!=='undefined'){this.propertyNames.push('evn_type');this.beforeValues.push(this.receiver['evn_type']);this.afterValues.push(options.changes.evn_type);}
if(typeof options.changes.evn_behavior!=='undefined'){this.propertyNames.push('evn_behavior');this.beforeValues.push(this.receiver['evn_behavior']);this.afterValues.push(options.changes.evn_behavior);}
if(typeof options.changes.evn_marker!=='undefined'){this.propertyNames.push('evn_marker');this.beforeValues.push(this.receiver['evn_marker']);this.afterValues.push(options.changes.evn_marker);}
if(typeof options.changes.evn_message!=='undefined'){this.propertyNames.push('evn_message');this.beforeValues.push(this.receiver['evn_message']);this.afterValues.push(options.changes.evn_message);}
break;case'changescripttypeactivity':this.beforeStyle.push(this.layers[0].zoomSprites);newZoom=[];for(i=0;i<this.beforeStyle[0].length;i+=1){newZoom.push('adam-shape-'+((i*25)+50)+'-activity-scripttask-'+options.changes.toLowerCase());}
this.afterStyle.push(newZoom);this.propertyNames.push('act_script_type');this.beforeValues.push(receiver['act_script_type']);this.afterValues.push(options.changes);break;}};AdamShapeLayerCommand.prototype.switchDefaultFlow=function(id,newVal,oldVal){var connection,updatedElement;connection=this.receiver.canvas.connections.find('id',id);connection.changeFlowType(newVal.toLowerCase());connection.setFlowType(newVal);updatedElement=[{id:connection.getID(),type:connection.type,relatedObject:connection,fields:[{field:"type",newVal:newVal,oldVal:oldVal}]}];this.receiver.getCanvas().triggerDefaultFlowChangeEvent(updatedElement);};AdamShapeLayerCommand.prototype.execute=function(){var i;for(i=0;i<this.layers.length;i+=1){this.layers[i].zoomSprites=this.afterStyle[i];this.layers[i].paint();}
for(i=0;i<this.propertyNames.length;i+=1){this.receiver[this.propertyNames[i]]=this.afterValues[i];if(this.propertyNames[i]==='gat_default_flow'){this.switchDefaultFlow(this.beforeValues[i],'SEQUENCE','DEFAULT');}
if(this.receiver.getType()==='AdamGateway'){this.receiver.updateFlowConditions();}}
if(this.afterName!==null){this.receiver.setName(this.afterName).label.textField.value=this.afterName;}
this.canvas.triggerCommandAdam(this.receiver,this.propertyNames,this.beforeValues,this.afterValues);};AdamShapeLayerCommand.prototype.undo=function(){var i;for(i=0;i<this.layers.length;i+=1){this.layers[i].zoomSprites=this.beforeStyle[i];this.layers[i].paint();}
for(i=0;i<this.propertyNames.length;i+=1){this.receiver[this.propertyNames[i]]=this.beforeValues[i];if(this.propertyNames[i]==='gat_default_flow'){this.switchDefaultFlow(this.beforeValues[i],'DEFAULT','SEQUENCE');}}
if(this.beforeName!==null){this.receiver.setName(this.beforeName).label.textField.value=this.beforeName;}
this.canvas.triggerCommandAdam(this.receiver,this.propertyNames,this.afterValues,this.beforeValues);};AdamShapeLayerCommand.prototype.redo=function(){this.execute();};var AdamShapeMarkerCommand=function(receiver,options){jCore.Command.call(this,receiver);this.updateType=null;this.markers=[];this.beforeMarkerStyle=[];this.beforeMarkerType=[];this.afterMarkerStyle=[];this.afterMarkerType=[];this.beforeValues=[];this.afterValues=[];this.propertyNames=[];AdamShapeMarkerCommand.prototype.initObject.call(this,receiver,options);};AdamShapeMarkerCommand.prototype=new jCore.Command();AdamShapeMarkerCommand.prototype.type='AdamShapeMarkerCommand';AdamShapeMarkerCommand.prototype.initObject=function(receiver,options){var i,newZoom;this.updateType=options.type;this.markers=options.markers;switch(this.updateType){case'changeactivitymarker':this.beforeMarkerStyle.push(this.markers[0].markerZoomClasses);newZoom=[];for(i=0;i<this.beforeMarkerStyle[0].length;i+=1){newZoom.push('adam-marker-'+((i*25)+50)+'-'+options.changes.taskType.toLowerCase());}
this.afterMarkerStyle.push(newZoom);this.beforeMarkerType.push(this.receiver['act_task_type']);this.afterMarkerType.push(options.changes.taskType);this.propertyNames.push('act_task_type');this.beforeValues.push(this.receiver['act_task_type']);this.afterValues.push(options.changes.taskType);break;}};AdamShapeMarkerCommand.prototype.execute=function(){var i;for(i=0;i<this.markers.length;i+=1){this.markers[i].markerType=this.afterMarkerType[i];this.markers[i].markerZoomClasses=this.afterMarkerStyle[i];this.markers[i].paint(true);}
for(i=0;i<this.propertyNames.length;i+=1){this.receiver[this.propertyNames[i]]=this.afterValues[i];}
this.canvas.triggerCommandAdam(this.receiver,this.propertyNames,this.beforeValues,this.afterValues);};AdamShapeMarkerCommand.prototype.undo=function(){var i;for(i=0;i<this.markers.length;i+=1){this.markers[i].markerType=this.beforeMarkerType[i];this.markers[i].markerZoomClasses=this.beforeMarkerStyle[i];this.markers[i].paint(true);}
for(i=0;i<this.propertyNames.length;i+=1){this.receiver[this.propertyNames[i]]=this.beforeValues[i];}
this.canvas.triggerCommandAdam(this.receiver,this.propertyNames,this.afterValues,this.beforeValues);};AdamShapeMarkerCommand.prototype.redo=function(){this.execute();};
/* End of File include/javascript/pmse/command.js */

var CommandAnnotationResize=function(receiver){jCore.CommandResize.call(this,receiver);};CommandAnnotationResize.prototype.type='commandAnnotationResize';CommandAnnotationResize.prototype.execute=function(){jCore.CommandResize.prototype.execute.call(this);this.receiver.paint();};CommandAnnotationResize.prototype.undo=function(){jCore.CommandResize.prototype.undo.call(this);this.receiver.paint();};CommandAnnotationResize.prototype.redo=function(){this.execute();};
/* End of File include/javascript/pmse/command_annotation_resize.js */
...
/* End of File include/javascript/pmse/command_single_property.js */
...
App.alert.dismiss('upload');w.html.style.display='inline';}});}});project.addMetadata("teams",{dataURL:project.getMetadata("teamsDataSource").url,dataRoot:project.getMetadata("teamsDataSource").root,success:function(data){var i;if(emailPickerFields.length){for(i=0;i<emailPickerFields.length;i+=1){f.items[emailPickerFields[i]].setTeamTextField("text");f.items[emailPickerFields[i]].setTeams(data);}}else{for(i=0;i<f.items.length;i+=1){switch(f.items[i].name){case'address_to':case'address_cc':case'address_bcc':f.items[i].setTeamTextField("text");f.items[i].setTeams(data);break;}}}}});project.addMetadata("roles",{dataURL:'pmse_Project/CrmData/rolesList',dataRoot:"result",success:function(data){var i;if(emailPickerFields.length){for(i=0;i<emailPickerFields.length;i+=1){f.items[emailPickerFields[i]].setRoleTextField("text");f.items[emailPickerFields[i]].setRoles(data);}}else{for(i=0;i<f.items.length;i+=1){switch(f.items[i].name){case'address_to':case'address_cc':case'address_bcc':f.items[i].setRoleTextField("text");f.items[i].setRoles(data);break;}}}}});auxProxy=new SugarProxy({url:'pmse_Project/CrmData/related/'+PROJECT_MODULE});auxProxy.getData({cardinality:'all'},{success:function(data){var i;data=data.result;data.unshift({value:"",text:"Select..."});if(emailPickerFields.length){for(i=0;i<emailPickerFields.length;i+=1){f.items[emailPickerFields[i]].setModules(data);}}else{for(i=0;i<f.items.length;i+=1){switch(f.items[i].name){case'address_to':case'address_cc':case'address_bcc':f.items[i].setModules(data);break;}}}}});},submit:function(data){}};}else{items=[this._makeCriteriaField()];wHeight=185;wWidth=690;callback={loaded:function(data){root.canvas.emptyCurrentSelection();App.alert.dismiss('upload');w.html.style.display='inline';}};}}
...
ddlEmailTemplate.proxy.uid=ddlModules.value;ddlEmailTemplate.proxy.url='pmse_Project/CrmData/emailtemplates/'+ddlModules.value;aTemplate=[{'text':translate('LBL_PMSE_FORM_OPTION_SELECT'),'value':''}];ddlEmailTemplate.proxy.getData(null,{success:function(params){aTemplate=aTemplate.concat(params.result);ddlEmailTemplate.setOptions(aTemplate);App.alert.dismiss('upload');w.html.style.display='inline';}});}});project.addMetadata("teams",{dataURL:project.getMetadata("teamsDataSource").url,dataRoot:project.getMetadata("teamsDataSource").root,success:function(data){var i;if(emailPickerFields.length){for(i=0;i<emailPickerFields.length;i+=1){f.items[emailPickerFields[i]].setTeamTextField("text");f.items[emailPickerFields[i]].setTeams(data);}}else{for(i=0;i<f.items.length;i+=1){switch(f.items[i].name){case'address_to':case'address_cc':case'address_bcc':f.items[i].setTeamTextField("text");f.items[i].setTeams(data);break;}}}}});project.addMetadata("roles",{dataURL:'pmse_Project/CrmData/rolesList',dataRoot:"result",success:function(data){var i;if(emailPickerFields.length){for(i=0;i<emailPickerFields.length;i+=1){f.items[emailPickerFields[i]].setRoleTextField("text");f.items[emailPickerFields[i]].setRoles(data);}}else{for(i=0;i<f.items.length;i+=1){switch(f.items[i].name){case'address_to':case'address_cc':case'address_bcc':f.items[i].setRoleTextField("text");f.items[i].setRoles(data);break;}}}}});auxProxy=new SugarProxy({url:'pmse_Project/CrmData/related/'+PROJECT_MODULE});auxProxy.getData({cardinality:'all'},{success:function(data){var i;data=data.result;data.unshift({value:"",text:"Select..."});if(emailPickerFields.length){for(i=0;i<emailPickerFields.length;i+=1){f.items[emailPickerFields[i]].setModules(data);}}else{for(i=0;i<f.items.length;i+=1){switch(f.items[i].name){case'address_to':case'address_cc':case'address_bcc':f.items[i].setModules(data);break;}}}}});},submit:function(data){}};}
break;}
f=new PMSE.Form({proxy:proxy,closeContainerOnSubmit:true,items:items,buttons:[{jtype:'submit',caption:translate('LBL_PMSE_BUTTON_SAVE'),cssClasses:['btn','btn-primary']},{jtype:'normal',caption:translate('LBL_PMSE_BUTTON_CANCEL'),handler:function(){$('.hasDatepicker').datepicker('hide');if(f.isDirty()){cancelInformation=new MessagePanel({title:"Confirm",wtype:'Confirm',message:translate('LBL_PMSE_MESSAGE_CANCEL_CONFIRM'),buttons:[{jtype:'normal',caption:translate('LBL_PMSE_BUTTON_YES'),handler:function(){cancelInformation.close();w.close();}},{jtype:'normal',caption:translate('LBL_PMSE_BUTTON_NO'),handler:function(){cancelInformation.close();}}]});cancelInformation.show();}else{w.close();}},cssClasses:['btn btn-invisible btn-link']}],callback:callback,language:PMSE_DESIGNER_FORM_TRANSLATIONS});w=new PMSE.Window({width:wWidth,height:wHeight,modal:true,title:translate('LBL_PMSE_FORM_TITLE_LABEL_EVENT')+': '+this.getName()});w.addPanel(f);if(this.evn_type==='BOUNDARY'){disabled=true;}
if(this.evn_type==='END'&&this.evn_marker!=='MESSAGE'){disabled=true;}
action=new PMSE.Action({text:translate('LBL_PMSE_CONTEXT_MENU_SETTINGS'),cssStyle:'adam-menu-icon-configure',handler:function(){root.saveProject(root,App,w);},disabled:disabled});return action;};AdamEvent.prototype.stringify=function(){var inheritedJSON=AdamShape.prototype.stringify.call(this),thisJSON={evn_marker:this.getEventMarker(),evn_message:this.getEventMessage(),evn_condition:this.evn_condition,evn_attached_to:this.evn_attached_to,evn_is_interrupting:this.evn_is_interrupting,evn_behavior:this.evn_behavior};$.extend(true,inheritedJSON,thisJSON);return inheritedJSON;};AdamEvent.prototype.getBaseURL=function(){return'pmse_Project/EventDefinition/';};AdamEvent.prototype.getValidationFunction=function(){switch(this.getEventType()){case'START':return this.callbackFunctionForStartEvent;case'INTERMEDIATE':switch(this.getEventMarker()){case'TIMER':return this.callbackFunctionForWaitEvent;case'MESSAGE':switch(this.evn_behavior){case'CATCH':return this.callbackFunctionForReceiveMessageEvent;case'THROW':return this.callbackFunctionForSendMessageEvent;}}
case'END':return this.callbackFunctionForEndEvent;}};AdamEvent.prototype.callbackFunctionForStartEvent=function(data,element,validationTools){validationTools.validateNumberOfEdges(null,0,1,null,element);if(!data.evn_params){validationTools.createError(element,'LBL_PMSE_ERROR_FIELD_REQUIRED','Applies to');}
element.validateStartOrReceiveMessageCriteriaBox(data,element,validationTools);};AdamEvent.prototype.callbackFunctionForWaitEvent=function(data,element,validationTools){validationTools.validateNumberOfEdges(1,null,1,1,element);if(data.evn_params==='fixed date'){element.validateWaitEventCriteriaBox(data,element,validationTools);}else if(data.evn_criteria==0){validationTools.createError(element,'LBL_PMSE_ERROR_WAIT_EVENT_ZERO_DURATION');}else if(!data.evn_params||!data.evn_criteria){validationTools.createError(element,'LBL_PMSE_ERROR_WAIT_EVENT_NO_PARAMETERS');}};AdamEvent.prototype.callbackFunctionForReceiveMessageEvent=function(data,element,validationTools){validationTools.validateNumberOfEdges(1,null,1,1,element);element.validateStartOrReceiveMessageCriteriaBox(data,element,validationTools);};AdamEvent.prototype.callbackFunctionForSendMessageEvent=function(data,element,validationTools){validationTools.validateNumberOfEdges(1,null,1,1,element);validationTools.validateAtom('TEMPLATE',null,null,data.evn_criteria,element,validationTools);element.validateSendMessageCriteriaBoxes(data,element,validationTools);};AdamEvent.prototype.callbackFunctionForEndEvent=function(data,element,validationTools){validationTools.validateNumberOfEdges(1,null,0,0,element);if(element.getEventMarker()==='MESSAGE'){validationTools.validateAtom('TEMPLATE',null,null,data.evn_criteria,element,validationTools);element.validateSendMessageCriteriaBoxes(data,element,validationTools);}};AdamEvent.prototype.validateStartOrReceiveMessageCriteriaBox=function(data,element,validationTools){var criteria=[];if(data.evn_criteria){criteria=JSON.parse(data.evn_criteria);}
element.checkForImpossibleLogic(element,validationTools,criteria);element.validateCriteriaBoxAtoms(element,validationTools,criteria);};AdamEvent.prototype.validateWaitEventCriteriaBox=function(data,element,validationTools){var criteria=[];if(data.evn_criteria){criteria=JSON.parse(data.evn_criteria);}
element.validateCriteriaBoxAtoms(element,validationTools,criteria);element.validateCorrectNumberOfDateObjects(element,validationTools,criteria);};AdamEvent.prototype.validateSendMessageCriteriaBoxes=function(data,element,validationTools){var field;var criteria=[];if(data.evn_params){criteria=JSON.parse(data.evn_params);}
if(!criteria.to||!criteria.to.length){validationTools.createWarning(element,'LBL_PMSE_ERROR_FIELD_REQUIRED','To');}
for(field in criteria){element.validateCriteriaBoxAtoms(element,validationTools,criteria[field],true);}};AdamEvent.prototype.validateCriteriaBoxAtoms=function(element,validationTools,criteria,sendEvent){var i;var atom;var module;for(i=0;i<criteria.length;i++){atom=criteria[i];module=sendEvent?atom.module:atom.expModule;if(atom.chainedRelationship){module=atom.chainedRelationship.moduleLabel;}
validationTools.validateAtom(sendEvent?atom.type:atom.expType,module,sendEvent?atom.field:atom.expField,sendEvent?atom.value:atom.expValue,element,validationTools);}};AdamEvent.prototype.validateCorrectNumberOfDateObjects=function(element,validationTools,criteria){var i;var atom;var datetimeCount=0;for(i=0;i<criteria.length;i++){atom=criteria[i];if(atom.expSubtype){if(atom.expSubtype.toUpperCase()==='DATETIME'||atom.expSubtype.toUpperCase()==='DATE'){datetimeCount++;}}}
if(datetimeCount!==1){validationTools.createError(element,'LBL_PMSE_ERROR_WAIT_EVENT_ONE_DATETIME');}};AdamEvent.prototype.checkForImpossibleLogic=function(element,validationTools,criteria){var evaluator;evaluator=new validationTools.CriteriaEvaluator();evaluator.emptyCriteriaIsTrue=element.getEventType()==='START'?true:false;evaluator.addOr(criteria.slice());if(evaluator.isAlwaysFalse()){validationTools.createError(element,'LBL_PMSE_ERROR_LOGIC_IMPOSSIBLE');}};
/* End of File include/javascript/pmse/event.js */
...
/* End of File include/javascript/pmse/artifact.js */

var PropertiesGrid=function(selector){this.element=null;PropertiesGrid.prototype.init.call(this,selector);};PropertiesGrid.prototype.type='propertiesGrid';PropertiesGrid.prototype.init=function(selector){this.element=$(selector);return this;};PropertiesGrid.prototype.load=function(setup){this.element.progrid(setup);return this;};PropertiesGrid.prototype.clear=function(){this.element.empty();return this;};PropertiesGrid.prototype.forceFocusOut=function(){try{this.element.find('input, select').trigger('focusout');}catch(e){}};PropertiesGrid.prototype.setWidth=function(width){this.element.progrid('setWidth',width);return this;};
/* End of File include/javascript/pmse/properties_grid.js */
...
/* End of File include/javascript/pmse/pmtree.js */

(function($){var defaultPrefix='progrid',defaultSettings={id:null,rows:[],width:'100%',fontSize:'11px',prefix:defaultPrefix,onValueChanged:null,onChangeDiscarded:null,onViewMode:null,onEditMode:null,onRowsInitialized:null,onRowSelected:null,onRowDeselected:null,onGridReady:null},instances=0,createRow=function(settings,prefix){if(!settings.name){$.error('name setting is required!');}
var defaultSettings=$.extend({label:'',type:'',readOnly:false,trueLabel:'true',falseLabel:'false',yesNoValueMode:'boolean',validate:'none'},settings),row={name:defaultSettings.name,label:defaultSettings.label,type:defaultSettings.type,readOnly:defaultSettings.readOnly},html=null,value=null,valueText=null,addSelectRowListener=function(flag){if(typeof flag==='undefined'||flag){$(html).find('td').on('selectRow',onSelectRow);$(html).on("click",function(){$(this).find("td:last").trigger("selectRow");});}else{$(html).find('td').off('selectRow',onSelectRow);$(html).off("click");}},addDeselectRowListener=function(flag){if(typeof flag==='undefined'||flag){$(html).find('td').on('focusout',onDeselectRow);}else{$(html).find('td').off('focusout',onDeselectRow);}},onSelectRow=function(){var row=$(this).parent(),obj=row.data("rowObject");addSelectRowListener(false);if(typeof obj.onSelected==='function'){obj.onSelected.call(obj);}
if(obj.readOnly){$(html).find('td:last div').focus();addDeselectRowListener();return;}
obj.editMode();addDeselectRowListener();},onDeselectRow=function(){var row=$(this).parent(),obj=row.data("rowObject");addDeselectRowListener(false);obj.editMode(false,false);addSelectRowListener();if(typeof obj.onRowDeselected==='function'){obj.onRowDeselected.call(obj);}},setViewModeEvents=function(){$(this.getHTML()).find("td:last div").on("focus",function(){$(this.parentNode).trigger("selectRow");}).on('keydown',function(e){e.stopPropagation();if(e.keyCode===13)
$(this.parentNode).trigger("selectRow");});},getCurrentSelectedValue=function(){var selectedValue=$(html).find('td:last > *').val();switch(this.type){case'text':selectedValue=defaultSettings.validate==='integer'?parseInt(selectedValue,10):selectedValue;break;case'yesNo':switch(defaultSettings.yesNoValueMode){case'int':selectedValue=parseInt(selectedValue,10);break;case'boolean':selectedValue=selectedValue==="true"?true:false;}
break;}
return selectedValue;},discardChanges=function(){if(!this.readOnly){var valueDiscarded=getCurrentSelectedValue.call(this),currentValue=this.getValue();this.editMode(false);if(typeof this.onChangeDiscarded==='function'&&valueDiscarded!==currentValue){this.onChangeDiscarded.call(this,{valueDiscarded:valueDiscarded,currentValue:currentValue});}}};row.onValueChanged=null;row.onSelected=null;row.onEditMode=null;row.onViewMode=null;row.onChangeDiscarded=null;row.onRowDeselected=null;row.getValue=function(){return value;};row.updateValueText=function(){valueText=this.getValue();switch(this.type){case'text':valueText=value.toString();break;case'yesNo':valueText=value?defaultSettings.trueLabel:defaultSettings.falseLabel;break;case'select':if(!defaultSettings.options){valueText="";break;}
for(i=0;i<defaultSettings.options.length;i++){if(defaultSettings.options[i].value===valueText){valueText=defaultSettings.options[i].label;break;}}
break;}};row.getValueText=function(){return valueText;};row.getHTML=function(){if(!html){var value=this.getValueText(),i,that=this;html=document.createElement('tr');$(html).append('<td class="'+prefix+'-first-col"><div style="width:0px">'+this.label+'</div></td>'+'<td class="'+prefix+'-second-col"><div tabindex="0" style="width:0px">'+(value?value:'')+'</div></td>');$(html).data("rowObject",this);addSelectRowListener(true);setViewModeEvents.call(this);}
return html;};row.save=function(){if(defaultSettings.readOnly){return;}
var previousValue=this.getValue(),data,newValue=getCurrentSelectedValue.call(this);if(defaultSettings.validate==='integer'&&this.type==='text'){previousValue=parseInt(previousValue,10);}
if(newValue!==previousValue){value=newValue;this.updateValueText();if(typeof this.onValueChanged==='function'){this.onValueChanged({row:this,newValue:newValue,previousValue:previousValue});}}
this.editMode(false);};row.editMode=function(edit,focus){var element,val,that=this,width=$(html.parentNode).width(),fontSize=parseInt($(html).css('font-size'),10),availableWidth=(width/2)-(fontSize*1.82);if(typeof edit==='undefined'||edit){switch(this.type){case'text':element=$('<input>').addClass('input-small').attr({type:"text"}).val(this.getValueText()).css({"width":availableWidth});if(defaultSettings.validate==='integer'){element.on('keydown',function(e){e.stopPropagation();if(e.keyCode==46||e.keyCode==8||e.keyCode==9||e.keyCode==27||e.keyCode==13||(e.keyCode==65&&e.ctrlKey===true)||(e.keyCode>=35&&e.keyCode<=39)){return;}
else{if(e.shiftKey||(e.keyCode<48||e.keyCode>57)&&(e.keyCode<96||e.keyCode>105)){e.preventDefault();}}});}
break;case'select':element=$('<select>').css({width:"100%"});for(i=0;i<this.options.length;i++){name=this.options[i].label;element.append($('<option label="'+name+'" value="'+this.options[i].value+'" '+((this.options[i].value===this.getValue())?'selected="selected"':'')+' '+(this.options[i].disabled?'disabled':'')+' >'+name+'</option>'));}
break;case'yesNo':if(typeof value==='string'){val=value;val=val==='true'?true:false;}else{val=!!value;}
element=$('<select>').css({width:"100%"});element.append($('<option label="'+defaultSettings.trueLabel+'" value="'+(defaultSettings.yesNoValueMode==='int'?1:'true')+'" '+(val?'selected="selected"':'')+'>'+defaultSettings.trueLabel+'</option>'));element.append($('<option label="'+defaultSettings.falseLabel+'" value="'+(defaultSettings.yesNoValueMode==='int'?0:'false')+'" '+(!val?'selected="selected"':'')+'>'+defaultSettings.falseLabel+'</option>'));}
element.on('focusout',function(){that.save();}).on("click",function(e){e.stopPropagation();}).on("keydown",function(e){e.stopPropagation();if(e.keyCode===13){that.save();}else if(e.keyCode===27){discardChanges.call(that);}});try{$(html).find('td:last').empty();}catch(e){};$(html).find('td:last').append(element);element.focus();if(typeof this.onEditMode==='function'){this.onEditMode.call(this);}
element.select();}else{try{$(html).find('td:last').empty();}catch(e){};$(html).find('td:last').append($('<div tabindex="0" style="width: '+availableWidth+'px">'+this.getValueText()+'</div>'));if(typeof focus==='undefined'||focus){$(html).find('td:last div').focus();}
setViewModeEvents.call(this);if(typeof this.onViewMode==='function'){this.onViewMode.call(this);}}};prefix=prefix||defaultPrefix;if(row.type==='select'){row.options=settings.options||[];}
switch(row.type){case'text':value=(typeof defaultSettings.value!=='undefined'&&defaultSettings.value!==null&&(defaultSettings.validate!=='int'?defaultSettings.value.toString():parseInt(defaultSettings.value,10)))||'';break;case'select':if(!defaultSettings.options){value=null;break;}
for(i=0;i<defaultSettings.options.length;i++){if((typeof defaultSettings.options[i].selected==='boolean'&&defaultSettings.options[i].selected)||(typeof defaultSettings.options[i].selected!=='boolean'&&defaultSettings.options[i].selected===defaultSettings.options[i].value)){value=defaultSettings.options[i].value;break;}}
if(value===null&&defaultSettings.options[0]){value=(defaultSettings.options&&defaultSettings.options[0].value)||null;}
break;case'yesNo':if(typeof defaultSettings.value==='string'){value=defaultSettings.value.toLowerCase();value=value==='true'?true:false;}else{value=!!defaultSettings.value;}
switch(defaultSettings.yesNoValueMode){case'int':value=value?1:0;break;case'literal':value=value?'true':'false';}}
row.updateValueText();return row;},setWidth=function(width){if(typeof width!=='undefined'){var theWidth;var fontSize=parseInt($(this).find('table.progrid-table').css("font-size"));if(isNaN(fontSize)){fontSize=0;}
theWidth=$(this).find('table').find('td div').css({"width":"0px"}).end().css({"width":width}).width();theWidth=(theWidth/2)-(1.82*fontSize);$(this).find('td div').css({"width":theWidth});}},appendRow=function(row){var i,j,aux,flag=false,rows=this.find('tr'),name;for(i=0;i<rows.length;i++){name=$(rows[i]).data('rowObject').label;for(j=0;j<name.length;j++){if(row.label===""||row.label.charAt(j)<name.charAt(j)){flag=true;}
break;}
if(flag){break;}}
if(rows[i]){$(rows[i]).before(row.getHTML());}else{this.append(row.getHTML());}},buildTable=function(settings){var $this=$(this),table=$(document.createElement('table')),i,row,body;table.css({"width":settings.width,"font-size":settings.fontSize}).addClass('progrid-table').attr({'id':settings.prefix+'-propTable-'+instances,cellpadding:0,cellspacing:0,border:0});table.append($('<thead>').append($('<tr>').append('<th>Name</th>').append('<th>Value</th>')));body=$('<tbody>');for(i=0;i<settings.rows.length;i++){row=createRow(settings.rows[i]);row.onSelected=function(){$(this.getHTML()).addClass('row_selected');if(typeof settings.onRowSelected==='function'){settings.onRowSelected.call($this.get(0),{id:settings.id,fieldName:this.name,fieldLabel:this.label,fieldType:this.type,value:this.getValue()});}};row.onViewMode=function(){if(typeof settings.onViewMode==='function'){settings.onViewMode.call($this.get(0),{id:settings.id,fieldName:this.name,fieldLabel:this.label,fieldType:this.type,value:this.getValue()});}};row.onEditMode=function(){if(typeof settings.onEditMode==='function'){settings.onEditMode.call($this.get(0),{id:settings.id,fieldName:this.name,fieldLabel:this.label,fieldType:this.type,value:this.getValue()});}};row.onRowDeselected=function(){$(this.getHTML()).removeClass('row_selected');if(typeof settings.onRowDeselected==='function'){settings.onRowDeselected.call($this.get(0),{id:settings.id,fieldName:this.name,fieldLabel:this.label,fieldType:this.type,value:this.getValue()});}};row.onChangeDiscarded=function(data){if(typeof settings.onChangeDiscarded==='function'){settings.onChangeDiscarded.call($this.get(0),{id:settings.id,fieldName:this.name,fieldLabel:this.label,fieldType:this.type,currentValue:data.currentValue,discardedValue:data.valueDiscarded});}};row.onValueChanged=function(data){if(typeof settings.onValueChanged==='function'){settings.onValueChanged.call($this.get(0),{id:settings.id,fieldName:this.name,fieldLabel:this.label,fieldType:this.type,value:data.newValue,previousValue:data.previousValue});}};$(row.getHTML()).addClass(i%2===0?'odd':'even');appendRow.call(body,row);}
table.append(body);if(typeof settings.onRowsInitialized==='function'){settings.onRowsInitialized.call($this.get(0),{rows:settings.rows});}
$this.append(table);},methods={init:function(settings){settings=$.extend({},defaultSettings,settings);return this.each(function(){instances++;$(this).empty();buildTable.call(this,settings);setWidth.call(this,settings.width);$(this).trigger('gridready');if(typeof settings.onGridReady==='function'){settings.onGridReady.call(this);}});},setWidth:function(width){setWidth.call(this,width);}};$.fn.progrid=function(method){if(methods[method]){return methods[method].apply(this,Array.prototype.slice.call(arguments,1));}else if(typeof method==='object'||!method){return methods.init.apply(this,arguments);}else{}};})(jQuery);
/* End of File include/javascript/pmse/progrid.js */

var PMSE=PMSE||{};var ErrorMessageItem=function(options){PMSE.Element.call(this,jQuery.extend(true,options,{position:"relative"}));this.message=null;this.messageId=null;this.messageContainer=null;this.parent=null;ErrorMessageItem.prototype.initObject.call(this,options);};ErrorMessageItem.prototype=new PMSE.Element();ErrorMessageItem.prototype.type="ErrorMessageItem";ErrorMessageItem.prototype.family="Element";ErrorMessageItem.prototype.initObject=function(options){var defaults={message:"[no message]",messageId:"",parent:null}
jQuery.extend(true,defaults,options);this.setMessage(defaults.message);this.setMessageId(defaults.messageId);this.setParent(defaults.parent);};ErrorMessageItem.prototype.setParent=function(parent){this.parent=parent;return this;};ErrorMessageItem.prototype.getParent=function(parent){return this.parent;};ErrorMessageItem.prototype.setMessageId=function(messageId){if(!(typeof messageId==="string")){throw new Error("ErrorMessageItem.setMessageId(): not valid, should be a string value");}
this.messageId=messageId;return this;};ErrorMessageItem.prototype.getMessageId=function(){return this.messageId;};ErrorMessageItem.prototype.setMessage=function(message){if(!(typeof message==="string")){throw new Error("ErrorMessageItem.setMessage(): not valid, should be a string value");}
this.message=message;if(this.html){this.messageContainer.textContent=this.message;}
return this;};ErrorMessageItem.prototype.getMessage=function(){return this.message;};ErrorMessageItem.prototype.createHTML=function(){var messageContainer;if(!this.html){this.html=this.createHTMLElement('li');this.html.id=this.id;this.style.applyStyle();this.style.addProperties({position:"relative",left:this.x,top:this.y,width:this.width,height:this.height,zIndex:this.zOrder});messageContainer=this.createHTMLElement('span');messageContainer.className="messageContainer";this.html.appendChild(messageContainer);this.messageContainer=messageContainer;this.setMessage(this.message);this.html.style.height="auto";this.html.style.width="auto";this.html.className="comment";this.html.style.padding="3px 3px 3px 0px";}
return this.html;};
/* End of File include/javascript/pmse/ErrorMessageItem.js */

var PMSE=PMSE||{};var ListContainer=function(options){PMSE.Container.call(this,options);ListContainer.prototype.initObject.call(this,options);};ListContainer.prototype=new PMSE.Container();ListContainer.prototype.type='ListContainer';ListContainer.prototype.family='ListContainer';ListContainer.prototype.initObject=function(options){};ListContainer.prototype.setItems=function(items){var i;this.clearItems();if(!(jQuery.isArray(items))){throw new Error("ListContainer.setItems(): the value is invalid, should be a type array");}
for(i=0;i<items.length;i+=1){this.addItem(items[i]);}
return this;};ListContainer.prototype.addItem=function(item){var newItem;if(item instanceof ErrorMessageItem){newItem=item;}else if(typeof item==="object"){newItem=new ErrorMessageItem(item);}else{throw new Error("ListContainer.addItem(): the value is invalid");}
this.items.push(newItem);if(this.html){this.messagecontainer.appendChild(newItem.getHTML());}
return this;};ListContainer.prototype.clearItems=function(){var i,length=this.items.length;for(i=0;i<length;i+=1){this.removeItem(0)}
return this;};ListContainer.prototype.removeItem=function(index){var item=this.items.splice(index,1)[0];if(item.html){jQuery(item.getHTML()).remove();}
return this;};ListContainer.prototype.paintItems=function(){var i;if(this.messagecontainer){for(i=0;i<this.items.length;i+=1){this.body.appendChild(this.items[i].getHTML());}}
return this;};ListContainer.prototype.createHTML=function(){if(!this.html){PMSE.Container.prototype.createHTML.call(this);this.html.style.position="relative"}
return this.html;};ListContainer.prototype.getItems=function(){return this.items;};ListContainer.prototype.getItem=function(index){if(index>=0&&index<this.items.length){return this.items[index];}else{throw new Error("ListContainer.getItem():the index does not exist");}};
/* End of File include/javascript/pmse/ListContainer.js */

var ErrorListItem=function(options){ListContainer.call(this,options);this.messagecontainer=null;this.iconContainer=null;this.titleContainer=null;this.errorType=null;this.errorId=null;this.title=null;this.onClick=null;this.parent=null;this.listOfTypes={AdamGatewayEVENTBASED:"adam-tree-icon-gateway-exclusive",AdamGatewayINCLUSIVE:"adam-tree-icon-gateway-exclusive",AdamEventSTARTLeads:"adam-tree-icon-start-leads",AdamActivityUSERTASK:"adam-tree-icon-user-task",AdamEventSTARTOpportunities:"adam-tree-icon-start-opportunities",AdamEventSTARTDocuments:"adam-tree-icon-start-documents",AdamEventSTART:"adam-tree-icon-start",AdamGatewayEXCLUSIVE:"adam-tree-icon-gateway-exclusive",AdamGatewayPARALLEL:"adam-tree-icon-gateway-parallel",AdamEventINTERMEDIATETIMER:"adam-tree-icon-intermediate-timer",AdamEventENDEMPTY:"adam-tree-icon-end",AdamEventINTERMEDIATEMESSAGE:"adam-tree-icon-intermediate-message",textannotation:"adam-tree-icon-textannotation ",AdamEventSTARTMESSAGE:"adam-tree-icon-start",AdamActivitySCRIPTTASK:"adam-tree-icon-user-task"};ErrorListItem.prototype.initObject.call(this,options);};ErrorListItem.prototype=new ListContainer();ErrorListItem.prototype.type='ErrorListItem';ErrorListItem.prototype.family='ErrorListItem';ErrorListItem.prototype.initObject=function(options){var defaults={errorType:"",errorId:"",title:"[untitle]",onClick:null,parent:null};jQuery.extend(true,defaults,options);this.setErrorType(defaults.errorType);this.setErrorId(defaults.errorId);this.setTitle(defaults.title);this.setOnClick(defaults.onClick);this.setParent(defaults.parent);};ErrorListItem.prototype.setParent=function(parent){this.parent=parent;return this;};ErrorListItem.prototype.getParent=function(){return this.parent;};ErrorListItem.prototype.setOnClick=function(handler){if(!(typeof handler==='function'||handler===null)){throw new Error("ErrorListItem.setInconHandler(): the value is invalid");}
...
return this;};ErrorListItem.prototype.setTitle=function(title){if(!(typeof title==="string")){throw new Error("ErrorListItem.setTitle(): the value is invalid");}
this.title=title;if(this.html){this.titleContainer.textContent=this.title;this.resizeWidthTitle();}
return this;};ErrorListItem.prototype.resizeWidthTitle=function(){var auxWidth1,auxWidth2;if(this.html){auxWidth1=jQuery(this.titleContainer).outerWidth();this.titleContainer.style.width="auto";auxWidth2=jQuery(this.titleContainer).outerWidth();if(auxWidth2>auxWidth1){this.titleContainer.title=this.title;}else{this.titleContainer.title="";}
this.titleContainer.style.width="80%";}
return this;};ErrorListItem.prototype.getTitle=function(){return this.title;};ErrorListItem.prototype.setErrorId=function(id){if(!(typeof id==="string")){throw new Error("ErrorListItem.addItem(): the value is invalid");}
this.errorId=id;return this;};ErrorListItem.prototype.getErrorId=function(){return this.errorId;};ErrorListItem.prototype.createHTML=function(){var messagecontainer,iconContainer,titleContainer;if(!this.html){ListContainer.prototype.createHTML.call(this);messagecontainer=this.createHTMLElement('ul');messagecontainer.className="messagecontainer comments ";messagecontainer.style.margin="0 0 9px 25px";iconContainer=this.createHTMLElement('i');iconContainer.className="iconContainer";titleContainer=this.createHTMLElement('span');titleContainer.className="titleContainer adam-error-color";this.body.appendChild(iconContainer);this.body.appendChild(titleContainer);this.body.appendChild(messagecontainer);this.messagecontainer=messagecontainer;this.iconContainer=iconContainer;this.titleContainer=titleContainer;this.paintItems();this.setErrorType(this.errorType);this.setTitle(this.title);this.html.style.height="auto";this.attachListeners();$(this.html).addClass('activitystream-posts-comments-container');this.html.style.padding="8px";this.html.style.width="auto";this.html.style.height="auto";this.titleContainer.style.paddingLeft="10px";this.fixedStyles();}
return this.html;};ErrorListItem.prototype.fixedStyles=function(){if(this.html){jQuery(this.titleContainer).css({"width":"80%","text-overflow":"ellipsis","white-space":"nowrap","overflow":"hidden","display":"inline-block","cursor":"pointer"});}
return this;}
ErrorListItem.prototype.paintItems=function(){var i;if(this.messagecontainer){for(i=0;i<this.items.length;i+=1){this.messagecontainer.appendChild(this.items[i].getHTML());}}
return this;};ErrorListItem.prototype.setErrorType=function(errorType){if(!(typeof errorType==="string")){throw new Error("ErrorListItem.setErrorType(): not valid, should be a string value");}
this.errorType=errorType;if(this.html){jQuery(this.html).removeClass();jQuery(this.html).addClass("error-"+errorType);this.iconContainer.className=this.listOfTypes[errorType];}
return this;};ErrorListItem.prototype.addItem=function(item){var newItem;if(item instanceof ErrorMessageItem){newItem=item;}else if(typeof item==="object"){newItem=new ErrorMessageItem(item);}else{throw new Error("ErrorListItem.addItem(): the value is invalid");}
newItem.setParent(this);this.items.push(newItem);if(this.html){this.messagecontainer.appendChild(newItem.getHTML());}
return this;};ErrorListItem.prototype.getItemByMessageId=function(messageId){var i,item;for(i=0;i<this.items.length;i+=1){if(this.items[i].getMessageId()===messageId){item=this.items[i];}}
if(item){return item;}else{null;}};
/* End of File include/javascript/pmse/ErrorListItem.js */

var ErrorListPanel=function(options){ListContainer.call(this,options);this.onClickItem=null;this.title=null;this.parent=null;this.titleContainer=null;this.selectedItem=null;this.classItemSelected=null;ErrorListPanel.prototype.initObject.call(this,options);};ErrorListPanel.prototype=new ListContainer();ErrorListPanel.prototype.type='ErrorListPanel';ErrorListPanel.prototype.family='ErrorListPanel';ErrorListPanel.prototype.initObject=function(options){var defaults={onClickItem:null,title:"[Untitle]",parent:null,classItemSelected:"selected"}
jQuery.extend(true,defaults,options);this.setOnClickItem(defaults.onClickItem);this.setTitle(defaults.title);this.setParent(defaults.parent);this.setClassItemSelected(defaults.classItemSelected);};ErrorListPanel.prototype.setClassItemSelected=function(className){if(!(typeof className==="string")){throw new Error("ErrorListPanel.setClassItemSelected:the value is invalid ");}
this.classItemSelected=className;return this;};ErrorListPanel.prototype.getClassItemSelected=function(){return this.classItemSelected;};ErrorListPanel.prototype.setParent=function(parent){this.parent=parent;return this;};ErrorListPanel.prototype.getParent=function(){return this.parent;};ErrorListPanel.prototype.setTitle=function(title){if(!(typeof title==="string")){throw new Error("ErrorListPanel.setTitle():the value is invalid ");}
this.title=title;if(this.html){this.titleContainer.textContent=title;}
return this;};ErrorListPanel.prototype.getTitle=function(){return this.title;};ErrorListPanel.prototype.setOnClickItem=function(handler){var i;if(!(typeof handler==='function'||handler===null)){throw new Error("ErrorListPanel.setInconHandler(): the value is invalid");}
this.onClickItem=handler;if(this.items.length){for(i=0;i<this.items.length;i+=1){this.items[i].onClick=this.onClickItem;}}
return this;};ErrorListPanel.prototype.createHTML=function(){var titleContainer;if(!this.html){ListContainer.prototype.createHTML.call(this);titleContainer=this.createHTMLElement('h4');titleContainer.className="dashlet-title adam-error-color";this.html.appendChild(titleContainer);this.titleContainer=titleContainer;jQuery(this.body).remove();body=this.createHTMLElement('div');body.className='j-container';this.html.appendChild(body);this.body=body;this.setBodyHeight(this.bodyHeight);this.paintItems();this.setTitle(this.title);this.customStyles();}
return this.html;};ErrorListPanel.prototype.customStyles=function(){if(this.html){this.body.style.listStyle="none";this.titleContainer.style.margin="0px";this.titleContainer.style.padding="6px 5px 6px 10px";this.titleContainer.style.fontWeight=500;this.titleContainer.style.background="#f6f6f6";this.titleContainer.style.borderBottom="1px solid #ddd";this.html.style.width="auto";this.html.style.background="white";this.html.style.height="auto";this.html.style.border="1px solid #ddd";jQuery(this.html).css("borderRadius","3px");jQuery(this.titleContainer).css("borderRadius","3px 3px 0px 0px");}
return this;};ErrorListPanel.prototype.paintItems=function(){var i;if(this.html){for(i=0;i<this.items.length;i+=1){this.body.appendChild(this.items[i].getHTML());}}
return this;};ErrorListPanel.prototype.addItem=function(item){var newItem;if(item instanceof ErrorListItem){newItem=item;}else if(typeof item==="object"){newItem=new ErrorListItem(item);}else{throw new Error("ErrorListPanel.addItem(): the value is invalid");}
newItem.setParent(this);newItem.onClick=this.onClickItem;this.items.push(newItem);if(this.html){this.body.appendChild(newItem.getHTML());}
...
return this;};
/* End of File include/javascript/pmse/ErrorListPanel.js */
...
if(maxOutgoing&&outgoingEdges>maxOutgoing){createWarning(element,'LBL_PMSE_ERROR_FLOW_OUTGOING_MAXIMUM',maxOutgoing);}};var validateAtom=function(type,module,field,value,element,validationTools){var i;var searchInfo=getSearchInfo(type,module,field,value);var options={'bulk':'validate_element_settings'};if(searchInfo.url){validationTools.progressTracker.incrementTotalValidations();App.api.call('read',searchInfo.url,null,{success:function(data){for(i=0;i<data.result.length;i++){if(data.result[i].value===searchInfo.key){return;}}
if(searchInfo.backupSearchFunction&&searchInfo.backupSearchFunction()){return;}
createWarning(element,'LBL_PMSE_ERROR_DATA_NOT_FOUND',searchInfo.text);},error:function(data){createWarning(element,'LBL_PMSE_ERROR_DATA_NOT_FOUND',searchInfo.text);},complete:function(data){validationTools.progressTracker.incrementValidated();}},options);}};var getSearchInfo=function(type,module,field,value){var url;var text;var key;var backupSearchFunction;switch(type){case'MODULE':key=field;case'VARIABLE':case'recipient':url=App.api.buildURL('pmse_Project/CrmData/fields/'+module+'?base_module='+getTargetModule());text='Module field';key=key||value;backupSearchFunction=function(){var fields=App.metadata.getModule(module.charAt(0).toUpperCase()+module.slice(1)).fields;for(fieldName in fields){if(fieldName===key){return true;}}};break;case'USER_IDENTITY':url=App.api.buildURL('pmse_Project/CrmData/users/');text='User';key=value;break;case'USER_ROLE':case'role':url=App.api.buildURL('pmse_Project/CrmData/rolesList/');text='Role';key=value;break;case'RELATIONSHIP':key=value;case'user':url=App.api.buildURL('pmse_Project/CrmData/related/'+getTargetModule());text='Module relationship';key=key||module;break;case'TEAM':case'team':url=App.api.buildURL('pmse_Project/CrmData/teams/public/');text='Team';key=value;break;case'CONTROL':url=App.api.buildURL('pmse_Project/CrmData/activities/'+project.uid);text='Form activity';key=field;break;case'ALL_BUSINESS_RULES':url=App.api.buildURL('pmse_Project/CrmData/rulesets/'+project.uid);text='Business rule';key=value;break;case'BUSINESS_RULES':url=App.api.buildURL('pmse_Project/CrmData/businessrules/'+project.uid);text='Business rule action';key=field;break;case'TEMPLATE':url=App.api.buildURL('pmse_Project/CrmData/emailtemplates/'+getTargetModule());text='Email template';key=value;break;};return{url:url,text:text,key:key,backupSearchFunction:backupSearchFunction};};var createWarning=function(element,warningLabel,field){createError(element,warningLabel,field,true);};var createError=function(element,errorLabel,field,warning){var errorName=field?(translate(errorLabel)+': '+field):translate(errorLabel);var errorInfo=translate(errorLabel+'_INFO');var newRow=createErrorRow(element);var nameCell=newRow.insertCell(0);var errorCell=newRow.insertCell(1);nameCell.appendChild(createErrorName(element));errorCell.appendChild(createErrorIcon(warning));errorCell.appendChild(createErrorText(errorName,errorInfo));if(warning){element.hasWarning=true;}else{element.hasError=true;}};var createErrorRow=function(element){var rowNumber;var otherElement;for(rowNumber=0;rowNumber<currentErrorTable.rows.length;rowNumber++){otherElementName=currentErrorTable.rows[rowNumber].cells[0].innerText;if(element.getName()<otherElementName){break;}}
...
return true;};};
/* End of File include/javascript/pmse/designer.js */
